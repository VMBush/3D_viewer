...
int count_VF(work_struct *data, FILE *file) {
  ...
  while (fgets(str_file, sizeof(str_file), file)) {
    if (str_file[0] != '#') {
      if (check_symb(str_file, 'v')) {
        ...
        int v1, v2, v3;
        error = sscanf(str_file, "f %d/%*d/%*d %d/%*d/%*d %d/%*d/%*d", &v1, &v2,
                       &v3);
        if (error == 3) {
          data->amount_index += 3;
          data->amount_edges += 3;
  ...
  error = (data->amount_coord > 0 && data->amount_index > 0)
              ? OK
              : ERROR_FILE_STRUCT;
  rewind(file);
  return error;
...
int add_points(work_struct *data, FILE *file) {
  ...
  while (error == OK &&
         (count_coord < data->amount_coord ||
          count_index < data->amount_index ||
          count_edges < data->amount_edges) &&
